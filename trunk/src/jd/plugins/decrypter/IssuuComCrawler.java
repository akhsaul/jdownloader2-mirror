...
public class IssuuComCrawler extends PluginForDecrypt {
    public IssuuComCrawler(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    public ArrayList<DownloadLink> decryptIt(final CryptedLink param, ProgressController progress) throws Exception {
        ...
        br.getPage("https://issuu.com/api/content-service/public.reader.dynamic?input=%7B%22json%22%3A%7B%22type%22%3A%22user%22%2C%22username%22%3A%22" + ownerUsername + "%22%2C%22documentName%22%3A%22" + documentURI + "%22%7D%7D");
        if (isOffline(br)) {
            /* E.g. {"message":"Document does not exist"} */
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
        final Map<String, Object> resp1 = restoreFromString(br.getRequest().getHtmlCode(), TypeRef.MAP);
        final Map<String, Object> metadata = (Map<String, Object>) JavaScriptEngineFactory.walkJson(resp1, "result/data/json/metadata");
        /**
         * 2026-01-15: New: https://publication.issuu.com/<ownerUsername>/<documentURI>/reader4.json <br>
         * Both old and new endpoints work fine.
         */
        br.getPage("https://reader3.isu.pub/" + ownerUsername + "/" + documentURI + "/reader3_4.json");
        Map<String, Object> resp2 = restoreFromString(br.getRequest().getHtmlCode(), TypeRef.MAP);
        final Map<String, Object> document = (Map<String, Object>) resp2.get("document");
