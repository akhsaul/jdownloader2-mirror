...
public class VKontakteRuHoster extends PluginForHost {
    ...
    public static final String  VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK                            = "VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK";
    ...
    private String setAndGetFileNameFromDirectURL(final DownloadLink link, final String url) {
        String filename = Plugin.extractFileNameFromURL(url);
        if (filename != null) {
            try {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link, final Account account, final boolean isDownload) throws Exception {
        ...
        if (link.getPluginPatternMatcher().matches(TYPE_DIRECT)) {
            ...
            if (linkOk(link, finalurl, isDownload) != 1) {
            ...
            if (VKontakteRu.isDocument(link.getPluginPatternMatcher())) {
                ...
                final String storedDirecturl = this.getStoredDirecturl(link);
                if (audioIsValidDirecturl(finalurl) && linkOk(link, storedDirecturl, isDownload) != 1) {
                    finalurl = storedDirecturl;
                } else {
                    ...
                    if (postID != null && fromId != null) {
                        ...
                        if (finalurl != null) {
                            ...
                            finalurl = (String) JavaScriptEngineFactory.jsonToJavaObject(finalurl);
                        }
                    }
                    if (finalurl == null) {
                ...
                if (storedDirecturl != null && linkOk(link, storedDirecturl, isDownload) == 1) {
                    logger.info("Re-using stored directurl: " + storedDirecturl);
                    finalurl = storedDirecturl;
                } else {
                    /* Refresh directlink */
                    final VKontakteRu crawler = (VKontakteRu) this.getNewPluginForDecryptInstance(this.getHost());
                    if (this.isHLS(link, storedDirecturl)) {
                    ...
                    if (!StringUtils.isEmpty(chosenQuality)) {
                        ...
                        for (final DownloadLink result : results) {
                            ...
                            if (quality.equalsIgnoreCase(chosenQuality)) {
                                finalurl = result.getStringProperty(PROPERTY_GENERAL_directlink);
                                break;
                            }
                        }
                        if (finalurl == null) {
                        ...
                        finalurl = this.getStoredDirecturl(link);
                    }
                }
            } else {
                /* Single photo --> Complex handling */
                finalurl = this.getStoredDirecturl(link);
                if (finalurl == null && this.getPluginConfig().getBooleanProperty(VKWALL_STORE_PICTURE_DIRECTURLS, default_VKWALL_STORE_PICTURE_DIRECTURLS)) {
                    finalurl = getHighestQualityPicFromSavedJson(link, link.getStringProperty(PROPERTY_PHOTOS_directurls_fallback, null), isDownload);
                }
                if (finalurl == null) {
                    /* Try to obtain fresh directurl */
                    String photo_list_id = this.getPhotoListID(link);
                    ...
                    if (br.containsHTML("(?i)>\\s*(Unknown error|Unbekannter Fehler|Access denied|Error<)") || this.br.getHttpConnection().getResponseCode() == 404) {
                        ...
                        /*
                         * E.g. redirect to somewhere else --> Content is offline or we're lacking the permissions (e.g. account required).
                         */
                        throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
                    }
                    if (photo_list_id == null) {
                        ...
                        // TODO: Check this
                        // if (photo_list_id != null) {
                        // link.setProperty(PROPERTY_PHOTOS_photo_list_id, photo_list_id);
                        // }
                    }
                    if (albumID == null) {
                        ...
                        if (albumID == null) {
                            ...
                            if (jsonInsideHTML != null) {
                                final String albumIDFromJson = PluginJSonUtils.getJsonValue(jsonInsideHTML, "list");
                                if (!StringUtils.isEmpty(albumIDFromJson)) {
                                    /* Fix id */
                                    albumID = albumIDFromJson.replace("album", "");
                                }
                            }
                        }
                        if (albumID != null) {
                    ...
                    final String jsonWithImageQualities = br.getRegex("(\\{\"zFields.*?\\})\\);").getMatch(0);
                    if (jsonWithImageQualities != null) {
                        /* 2023-11-24 */
                        final Map<String, Object> photojsonroot = restoreFromString(jsonWithImageQualities, TypeRef.MAP);
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (!sizeAltMapping.containsKey(entry.getKey())) {
                                /* Skip invalid/unsupported items */
                                continue;
                            }
    ...
    private String getStoredDirecturl(final DownloadLink link) {
        String url = link.getStringProperty(PROPERTY_GENERAL_directlink);
        if (url == null) {
            /* Legacy compatibility for images added up until including revision 48531. */
            url = link.getStringProperty("picturedirectlink");
        }
        return url;
    }
    ...
    public void handleDownload(final DownloadLink link, final Account account) throws Exception, PluginException {
        ...
        if (link.getPluginPatternMatcher().matches(TYPE_DIRECT)) {
            ...
            finalurl = getStoredDirecturl(link);
        }
        if (this.isHLS(link, finalurl)) {
            /* HLS download */
    ...
    private String modifyFinalDownloadurl(final String input) {
        ...
        if (this.getPluginConfig().getBooleanProperty(VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK, default_VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK) && (photo_url_regex = new Regex(input, Pattern.compile("(https?://[^/]+)/imp(?:f|g)/([^\\?]+)\\?.+", Pattern.CASE_INSENSITIVE))).patternFind()) {
            /* Modify URL according to: https://board.jdownloader.org/showpost.php?p=538448&postcount=3490 */
            final String new_url = photo_url_regex.getMatch(0) + "/" + photo_url_regex.getMatch(1);
            return new_url;
        } else {
            final Regex urlregex = new Regex(input, "(https?://cs(\\d+)\\.vk\\.me/)");
            if (!urlregex.patternFind()) {
                /* Return unmodified url */
                return input;
            }
            /* 2024-08-20: This is very old. I'm unsure whether this is still needed and working. */
            final String oldserver = urlregex.getMatch(0);
            final String serv_id = urlregex.getMatch(1);
            final String newserver = "https://pp.vk.me/c" + serv_id + "/";
            return input.replace(oldserver, newserver);
        }
    }
     ...
     *         Server 404 response
     */
    private int linkOk(final DownloadLink link, String finalUrl, final boolean isDownload) throws Exception {
        if (StringUtils.isEmpty(finalUrl)) {
            return 0;
        }
    ...
    private boolean photolinkOk(final DownloadLink link, String finalfilename, final boolean isDownload, String directurl) throws Exception {
        ...
        final Browser br2 = this.br.cloneBrowser();
        directurl = modifyFinalDownloadurl(directurl);
        br2.getHeaders().put("Accept-Encoding", "identity");
        final PluginForHost orginalPlugin = link.getLivePlugin();
        if (!isDownload) {
            link.setLivePlugin(this);
    ...
    private void postPageSafe(final Browser br, final Account acc, final DownloadLink link, final String page, final UrlQuery query) throws Exception {
        postPageSafe(br, acc, link, page, query.toString());
    }
    ...
    public static String getHighestQualityPicFromSavedJson(String picture_preview_json) {
        ...
        if (Encoding.isHtmlEntityCoded(picture_preview_json)) {
            picture_preview_json = Encoding.htmlOnlyDecode(picture_preview_json);
        }
    ...
    public String getLinkID(final DownloadLink link) {
        if (link.getPluginPatternMatcher() == null) {
            ...
            final String vkprefix = "vkontakte://";
            final String ownerID = getOwnerID(link);
            final String contentID = getContentID(link);
            if (this.isTypeVideo(link.getPluginPatternMatcher())) {
                return vkprefix + ownerID + "_" + contentID + "_" + link.getStringProperty(PROPERTY_VIDEO_SELECTED_QUALITY, "noneYet");
            } else {
                if (ownerID != null && contentID != null) {
                    /* Non-video items */
                    return vkprefix + ownerID + "_" + contentID;
                } else {
                    /* Fallback */
                    return super.getLinkID(link);
                }
    ...
    private String getOwnerID(final DownloadLink dl) {
        String ownerID = dl.getStringProperty("owner_id", null); // Legacy
    ...
    public Object getFavIcon(String host) throws IOException {
        if (getHost().equals(host)) {
            ...
            final BufferedImage ret = FavIcons.download_FavIconTag(br, getHost());
            return ret;
        } else {
            return null;
        }
    ...
    public static final boolean  default_VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK                            = false;
    ...
    public void setConfigElements() {
        ...
        this.getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, this.getPluginConfig(), VKWALL_STORE_PICTURE_DIRECTURLS, "Store picture-directlinks?\r\nThis helps to download images which can only be viewed inside comments but not separately.\r\nThis may also speedup the download process.\r\n WARNING: This may use a lot of RAM if you add big amounts of URLs!").setDefaultValue(default_VKWALL_STORE_PICTURE_DIRECTURLS));
        this.getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, this.getPluginConfig(), VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK, "Modify image URLs in an attempt to download original image?").setDefaultValue(default_VKPHOTOS_DOWNLOAD_ORIGINAL_IMAGE_VIA_MAGIC_TRICK));
        this.getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_SEPARATOR));
