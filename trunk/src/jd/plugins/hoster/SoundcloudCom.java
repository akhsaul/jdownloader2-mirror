...
public class SoundcloudCom extends PluginForHost {
    ...
    private final static String  AUDIO_PREFERRED_STREAM_CODEC                                          = "AUDIO_PREFERRED_STREAM_CODEC_V2";
    ...
    public String getLinkID(DownloadLink link) {
        final String quality = link.getStringProperty(PROPERTY_chosen_quality);
        if (quality != null) {
            final String songid = link.getStringProperty(PROPERTY_track_id);
            final String preset = link.getStringProperty(PROPERTY_chosen_preset);
            return getHost() + "://" + songid + "/" + quality + "/" + preset;
        }
    ...
    public static int getExpectedAudioBitrate(final DownloadLink link) {
        ...
        if (PROPERTY_QUALITY_hq.equals(chosenQuality)) {
            return 256;
        } else {
            return 128;
        }
    ...
    private static AtomicReference<Boolean> FFMPEG_OPUS_SUPPORTED = new AtomicReference<Boolean>();

    private boolean isOpusSupported() {
        synchronized (FFMPEG_OPUS_SUPPORTED) {
            Boolean value = FFMPEG_OPUS_SUPPORTED.get();
            if (value == null) {
                final FFmpeg ffmpeg = new FFmpeg(null) {
                    @Override
                    public LogInterface getLogger() {
                        return SoundcloudCom.this.getLogger();
                    }
                };
                if (ffmpeg.isAvailable() && ffmpeg.isCompatible()) {
                    Set<FLAG> flags = ffmpeg.getSupportedFlags();
                    value = flags != null && flags.contains(AbstractFFmpegBinary.FLAG.OPUS);
                } else {
                    value = false;
                }
                FFMPEG_OPUS_SUPPORTED.set(value);
            }
            return Boolean.TRUE.equals(value);
        }
    }

    private Map<String, Object> getPreferredTranscoding(final DownloadLink link, List<Map<String, Object>> transcodings) throws PluginException {
        if (transcodings == null || transcodings.size() == 0) {
            return null;
        }
        ...
        for (PreferredStreamCodec codec : codecs) {
            ...
            for (final Map<String, Object> transcoding : transcodings) {
                ...
                if (preset.startsWith("abr_")) {
                    ...
                    continue;
                } else if (!protocol.equals(link.getStringProperty(PROPERTY_chosen_protocol, protocol))) {
                    /* Skip because: a different protocol has been chosen before */
                    continue;
                }
                switch (codec) {
                case AAC:
                ...
                case OPUS:
                    if (!isOpusSupported()) {
                        logger.info("opus codec not supported by used ffmpeg version!");
                        continue;
                    }
        ...
        for (Map<String, Object> next : allowedTranscodings) {
            ...
            final String nextQuality = next.get("quality").toString();
            final String bestQuality = best.get("quality").toString();
            if (!"hq".equals(bestQuality) && "hq".equals(nextQuality)) {
                // switch to hq/premium
                best = next;
                continue;
            }
            final String nextPreset = next.get("preset").toString();
            final String bestPreset = best.get("preset").toString();
            if (bestPreset.matches(".*(\\d+)k$") && !nextPreset.matches(".*(\\d+)k$")) {
                // switch to non abr
                best = next;
                continue;
            }
    ...
    public String getDirectlink(final Plugin plugin, final DownloadLink link, final Account account, final Browser br, Map<String, Object> json) throws Exception {
    ...
    public static String getFormattedFilename(final DownloadLink link) throws ParseException {
        ...
        String setsposition = link.getStringProperty(PROPERTY_setsposition);
        if (formattedFilename.contains("*playlistposition*")) {
            if (setsposition != null) {
                setsposition = setsposition.replaceAll("[^0-9]", "");// only keep numbers
            }
            formattedFilename = formattedFilename.replace("*playlistposition*", setsposition != null ? setsposition : "");
        }
        if (cfg.getBooleanProperty(SETS_ADD_POSITION_TO_FILENAME, defaultSETS_ADD_POSITION_TO_FILENAME) && setsposition != null) {
            if (!setsposition.endsWith(".")) {
    ...
    private String[] getPreferredStreamCodecStrings() {
        final PreferredStreamCodec[] enumValues = PreferredStreamCodec.values();
        final String[] ret = new String[enumValues.length];
        for (int i = 0; i < enumValues.length; i++) {
            ret[i] = enumValues[i].getLabel();
        }
        return ret;
    }
    ...
    public static PreferredStreamCodec getPreferredStreamCodec() {
        ...
        if (arrayPos >= 0 && arrayPos < PreferredStreamCodec.values().length) {
            ...
            return PreferredStreamCodec.values()[defaultArrayPosAUDIO_PREFERRED_STREAM_CODEC];
        }
    }
    ...
    private static final int     defaultArrayPosAUDIO_PREFERRED_STREAM_CODEC                                  = 0;
    ...
    public void resetDownloadlink(final DownloadLink link) {
        ...
        link.removeProperty(PROPERTY_chosen_codec);
        link.removeProperty(PROPERTY_chosen_preset);
        link.removeProperty(PROPERTY_chosen_protocol);
    }
