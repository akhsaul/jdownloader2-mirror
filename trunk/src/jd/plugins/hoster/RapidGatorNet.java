    ...
    private void handleDownloadWebsite(final DownloadLink link, final Account account) throws Exception {
        ...
        try {
            ...
            findDirecturl: if ((storedDirecturl = link.getStringProperty(directlinkproperty)) != null) {
                ...
                CaptchaHosterHelperInterface captchaHelper = null;
                ...
                if (cfg.isEnableFreeDownloadModeCaptchaDuringPreDownloadWait() && lastUsedCaptchaType != null) {
                    ...
                    if (CAPTCHA_TYPE_RECAPTCHA.equals(lastUsedCaptchaType)) { /* reCaptcha captcha */
                        captchaHelper = new CaptchaHelperHostPluginRecaptchaV2(this, br, "6LcSUAsUAAAAAKBeQQE893pf0Io66-mIeKWPl5yF");
                    } else if (CAPTCHA_TYPE_TURNSTILE.equals(lastUsedCaptchaType)) {
                        captchaHelper = new CaptchaHelperHostPluginCloudflareTurnstile(this, br, "0x4AAAAAACGwIXkmZ2lsGdCV");
                    } else {
                        captchaHelper = null;
                    ...
                    if (captchaHelper != null) {
                        this.waitBeforeInteractiveCaptcha(link, waitMillis, captchaHelper.getSolutionTimeout());
                ...
                if (captchaform != null) {
                    ...
                    try {
                        do {
                            captchaAttempts++;
                            if (CaptchaHelperHostPluginCloudflareTurnstile.containsCloudflareTurnstileClass(br)) {
                                if (captchaResponse == null || !(captchaHelper instanceof AbstractCloudflareTurnstileCaptcha)) {
                                    captchaHelper = new CaptchaHelperHostPluginCloudflareTurnstile(this, br);
                                    captchaResponse = captchaHelper.getToken();
                                }
                                this.getPluginConfig().setProperty(PROPERTY_LAST_USED_CAPTCHA_TYPE, CAPTCHA_TYPE_TURNSTILE);
                                captchaform.put("DownloadCaptchaForm[verifyCode]", Encoding.urlEncode(captchaResponse));
                                ...
                                if (captchaResponse == null || !(captchaHelper instanceof AbstractRecaptchaV2)) {
                                    captchaHelper = new CaptchaHelperHostPluginRecaptchaV2(this, br);
                                    captchaResponse = captchaHelper.getToken();
                                }
    ...
    public AccountInfo parseAPIAccountInfo(final Account account, final Map<String, Object> response_map, final AccountInfo ai) throws Exception {
        synchronized (account) {
            ...
            if (DebugMode.TRUE_IN_IDE_ELSE_FALSE) {
                ...
                ai.setStatus(account.getType().getLabel() + " | StatusForDevs: " + usermap.get("state_label"));
            }
            return ai;
        }
    }
