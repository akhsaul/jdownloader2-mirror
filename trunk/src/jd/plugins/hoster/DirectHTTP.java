 ...
 */
@HostPlugin(revision = "$Revision: 51589 $", interfaceVersion = 2, names = { "DirectHTTP", "http links" }, urls = { "directhttp://.+",
...
public class DirectHTTP extends antiDDoSForHost implements DownloadConnectionVerifier {
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception {
        ...
        if (downloadLink.getProperty(BYPASS_CLOUDFLARE_BGJ) != null) {
            logger.info("Apply Cloudflare image polish workaround");
            resume = false;
            chunks = 1;
        }
    ...
    protected String updateFilename(final DownloadLink downloadLink, URLConnectionAdapter urlConnection) throws Exception {
        ...
        if (StringUtils.isEmpty(fileName)) {
            ...
            fileName = (source = FILENAME_SOURCE.CUSTOM).getFilename(this, downloadLink, urlConnection, downloadLink.getStringProperty(FIXNAME, null));
        }
        if (StringUtils.isEmpty(fileName)) {
            fileName = (source = FILENAME_SOURCE.HEADER).getFilename(this, downloadLink, urlConnection);
            if (StringUtils.isEmpty(fileName)) {
                fileName = (source = FILENAME_SOURCE.URL).getFilename(this, downloadLink, urlConnection);
            }
    ...
    protected AvailableStatus requestFileInformation(final DownloadLink downloadLink, int retry, Set<String> optionSet) throws Exception {
        ...
        try {
            ...
            updateFilename(downloadLink, urlConnection);
            if (length >= 0) {
    ...
    private void setCustomHeaders(final Browser br, final DownloadLink downloadLink, Set<String> optionSet) throws IOException {
        ...
        if (rule != null && rule.isEnabled()) {
            ...
            rule.applyCookiesAndHeaders(br, url, false);
        }
    ...
    public static String createURLForThisPlugin(final String url) {
        return url == null ? null : (StringUtils.startsWithCaseInsensitive(url, "directhttp://") ? url : "directhttp://" + url);
    }
}