...
public abstract class XFileSharingProBasic extends antiDDoSForHost implements DownloadConnectionVerifier {
    ...
    protected boolean isVideohoster(final Browser br) {
    ...
    protected boolean isOffline(final DownloadLink link, final Browser br) {
        ...
        if (br.containsHTML("No such file")) {
            ...
            return true;
        } else if (br.containsHTML(">\\s*File Deleted\\s*<")) {
            return true;
        } else if (br.containsHTML(">\\s*The file was removed by")) {
            return true;
        } else if (br.containsHTML(">[^<>]*Reason for deletion:?") || br.containsHTML(">\\s*File has been removed due to copyright issues\\s*<")) {
            // DDownload with emoji: <strong>🗑️ Reason for deletion</strong>
            return true;
    ...
    protected boolean massLinkcheckerWebsite(final DownloadLink[] urls) {
        ...
        try {
            ...
            while (index < urls.length) {
                links.clear();
                /* Collect links that we are going to check. */
                ...
                if (links.isEmpty()) {
                    /* This should be a super rare edge case */
                    logger.info("All links were short-urls and thus cannot be checked via mass linkchecker");
                    break;
                }
                sb.delete(0, sb.length());
                for (final DownloadLink link : links) {
                    if (sb.length() > 0) {
                        /* Add URL-encoded newline */
                        sb.append("%0A");
                    }
                    sb.append(URLEncode.encodeURIComponent(this.getNormalizedDownloadURL(link)));
                }
                {
                    /* Check if the mass-linkchecker works and which check-type we have to use */
                    /* Build list of check types to try */
                    final List<String> checkTypesToTry = new ArrayList<String>();
                    if (checkTypeCurrent != null) {
                        ...
                        if (!checkTypesToTry.contains(checkTypeNew)) {
                            checkTypesToTry.add(checkTypeNew);
                        }
                        if (!checkTypesToTry.contains(checkTypeOld)) {
                            checkTypesToTry.add(checkTypeOld);
                        }
                    }
                    boolean foundUsableCheckType = false;
                    int checkTypeIndex = 0;
                    findUsableLinkcheckType: while (checkTypeIndex < checkTypesToTry.size()) {
                        checkTypeCurrent = checkTypesToTry.get(checkTypeIndex);
                        checkTypeIndex++;
                        /*
                         ...
                         */
                        final String checkURL = getMainPage(br) + "/?op=" + checkTypeCurrent;
                        ...
                        if (!this.supports_availablecheck_filesize_alt_fast()) {
                            ...
                            if (checkForm == null) {
                                ...
                                continue findUsableLinkcheckType;
                            }
                        } else {
                        ...
                        /*
                         * Some hosts will not display any error page but also we will not be able to find any of our checked file-IDs
                         * inside the html --> Use this to find out about non-working linkchecking method!
                         */
                        final String example_fuid = this.getFUIDFromURL(links.get(0));
                        final boolean isValidResponse = br.getHttpConnection().getResponseCode() != 404 && br.getURL().contains(checkTypeCurrent) && br.containsHTML(example_fuid);
                        if (!isValidResponse) {
                            logger.info("Failed to find check_files Status via checkType: " + checkTypeCurrent);
                            continue findUsableLinkcheckType;
                        }
                        foundUsableCheckType = true;
                        break findUsableLinkcheckType;
                    }
    ...
    protected String getFnameViaAbuseLink(final Browser br, final DownloadLink link) throws Exception {
        getPage(br, getMainPage(br) + "/?op=report_file&id=" + this.getFUIDFromURL(link), false);
        /*
    ...
    protected void handlePassword(final Form form, final DownloadLink link) throws PluginException {
        if (isPasswordProtectedHTML(this.br, form)) {
    ...
    protected void checkErrors(final Browser br, final String html, final DownloadLink link, final Account account) throws NumberFormatException, PluginException {
        if (new Regex(html, "(?i)>\\s*Wrong password").patternFind()) {
            if (link.isPasswordProtected()) {
                ...
                /*
                 * 2020-03-26: Extremely rare case: Either plugin failure or serverside failure e.g. URL is password protected but website
                 * does never ask for the password e.g. 2020-03-26: ddl.to. We cannot use link.getDownloadPassword() to check this because
                 * users can enter download passwords at any time no matter whether they're required/used or not.
                 */
        ...
        } else if (br.containsHTML(">\\s*Downloads disabled for this file")) {
            throw new PluginException(LinkStatus.ERROR_FATAL, "Uploader has disabled downloads for this file");
        } else if (br.containsHTML(">\\s*Downloads are disabled for your country")) {
    ...
    protected void checkErrorsLastResort(final Browser br, final DownloadLink link, final Account account) throws PluginException {
        ...
        final boolean isHTML = br.isHtml();
        ...
        } else if (br.getRequest().getHtmlCode().length() <= 100 && !isHTML) {
    ...
    protected AccountInfo fetchAccountInfoWebsite(final Account account) throws Exception {
        ...
        if (supports_lifetime_account() && is_lifetime_account(br)) {
            ...
            if (expire_milliseconds == null) {
                ...
                final long apiValidUntil = ai.getValidUntil();
                if (apiValidUntil >= expire_milliseconds.longValue() && apiValidUntil <= expire_milliseconds.longValue() + TimeUnit.DAYS.toMillis(1)) {
                    // api valid until has higher precision
                    ai.setValidUntil(apiValidUntil);
                } else if (apiValidUntil <= expire_milliseconds.longValue() && apiValidUntil >= expire_milliseconds.longValue() - TimeUnit.DAYS.toMillis(1)) {
                    // api valid until has higher precision
                    ai.setValidUntil(apiValidUntil);
                } else {
    ...
    public boolean loginWebsite(final DownloadLink link, final Account account, final boolean validateCookies) throws Exception {
        synchronized (account) {
            try {
                ...
                try {
                    /* TODO: Make this nicer, also try to remove that ugly try catch block. */
                    return handleLoginWebsite2FA(e, link, account, validateCookies);
                } catch (Exception e2) {
                    /* Clear cookies to ensure not to save cookies of an invalid/"half" login attempt */
                    if (e.getLinkStatus() == LinkStatus.ERROR_PREMIUM) {
                        account.clearCookies("");
                    } else if (e2 instanceof PluginException && ((PluginException) e2).getLinkStatus() == LinkStatus.ERROR_PREMIUM) {
                        account.clearCookies("");
                    }
                    throw e2;
                }
    ...
    protected boolean handleLoginWebsite2FA(final PluginException e, final DownloadLink link, final Account account, final boolean validateCookies) throws Exception {
        ...
        final InputField twoFAField = get2FALoginField(br, twoFAForm);
        if (twoFAField == null) {
            logger.warning("Failed to find 2FA fieldKey");
            throw e;
        }
        twoFactorMessage = br.getRegex("<div[^>]*class\\s*=\\s*\"email-verify-info\">(.*?)</div>").getMatch(0);
        if (twoFactorMessage != null) {
            twoFactorMessage = twoFactorMessage.replaceAll("<[^>]*>", "").replaceAll("\\s+", " ").trim();
        }
        ...
        if (!this.isLoggedin(br) || find2FALoginform(br) != null) {
            throw new AccountInvalidException(e, org.jdownloader.gui.translate._GUI.T.jd_gui_swing_components_AccountDialog_2FA_login_invalid());
        }
    ...
    protected Form find2FALoginform(final Browser br) {
        ...
        for (final Form form : forms) {
            ...
            final InputField field = get2FALoginField(br, form);
            if (field != null) {
                return form;
            }
        }
    ...
    protected boolean tryDownload(final Browser br, final DownloadLink link, final Account account, final Request req, final DOWNLOAD_ATTEMPT_FLAGS... flags) throws Exception {
        ...
        if (req.getRequestMethod() == RequestMethod.GET && StringUtils.containsIgnoreCase(url, ".m3u8")) {
            ...
            if (LinkCrawlerDeepInspector.looksLikeMpegURL(dl.getConnection())) {
                ...
                /* Follow request so we can parse the m3u8 playlist information down below. */
                ...
                if (br.getHttpConnection().getResponseCode() == 416) {
                    // requested a website/html with not satisfiable range
                    // load it again without range so following (plugin)flow can work on proper website/html
                    final Request withoutRange = req.cloneRequest();
                    withoutRange.getHeaders().remove(HTTPConstants.HEADER_REQUEST_RANGE);
    ...
    protected String getAPIKeyFromAccount(final Account account) {
        synchronized (account) {
            String apikey;
            if (this.enableAccountApiOnlyMode()) {
